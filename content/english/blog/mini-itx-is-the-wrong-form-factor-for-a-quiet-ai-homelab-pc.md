...
- **Fan size.** A small case only fits small, high-RPM fans, and small fans have to spin faster than large fans to move the same volume of air. Faster fans are louder fans.
...
The GPU carries the AI workload here, so the CPU's real job is staying quiet at idle. A Ryzen 5 7600, non-X and without 3D V-Cache, measured **around 20W idle** in independent testing, a figure that held across two separate sources.
...
## The power supply only needs to cover the real load

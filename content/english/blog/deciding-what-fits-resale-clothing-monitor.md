...
Only listings that come back MAYBE and have a usable image go to a second pass with a vision-capable model. But a MAYBE with no resolvable image doesn't get silently dropped, it just stays a MAYBE and gets surfaced anyway, at lower confidence. A parse error or malformed model output defaults the same way.

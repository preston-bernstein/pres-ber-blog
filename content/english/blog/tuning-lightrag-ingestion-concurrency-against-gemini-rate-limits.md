...
## A Gemini 429 fails the whole document instead of retrying it
...
## Fixing the embedding batch size mattered more than any concurrency change
...
## The proxy layer needs to absorb overshoot instead of failing documents

Concurrency limits are a best-effort guess at the ceiling, and best-effort guesses are sometimes wrong. The real fix doesn't come from a more precise guess. It comes from making the failure mode survivable when the guess is wrong.

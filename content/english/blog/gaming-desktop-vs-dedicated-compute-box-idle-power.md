...
Contention on a shared machine is invisible until it isn't. I'd rather not find out about it during something time-sensitive. That's a maintenance and stability argument, not a power argument.

## GPU-bound work stayed on the desktop, and that's a separate decision

I did not move everything off the desktop. Local LLM inference stayed exactly where it was, running through the existing GPU-arbitration broker — a deliberate choice, not an oversight.
...
If your desktop genuinely goes idle for long stretches, take the wattage math seriously. The payback period is short, and the number is real. Just do the arithmetic on your own machine's actual behavior, not on whatever some N100 does in a review.

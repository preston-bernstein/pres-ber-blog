...
ClamAV's own database misses a lot that other groups have already catalogued. [`clamav-unofficial-sigs`](https://github.com/extremeshok/clamav-unofficial-sigs) is a maintained aggregator that pulls in [Sanesecurity](https://sanesecurity.com/), SecuriteInfo, [URLhaus](https://urlhaus.abuse.ch/), and MalwarePatrol feeds and drops them straight into the same database directory clamd already reads. No changes to my scan gate's code, no new dependency in the pipeline logic. It's a cron job and a shared volume. This is the highest ratio of detection gained to effort spent of anything I added, because it's pure config and it widens the signature set clamd already checks against on every scan.

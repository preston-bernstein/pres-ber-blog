...
## Deploy drift isn't a worktree problem: it's a gap between git and live state
...
It did exactly what I told it to do when something breaks downstream: **preserve state over convenience**. But I still haven't fixed the part where nothing reminds me to go check for it after a run stops early. Right now that's a manual habit I have to remember to do myself.
